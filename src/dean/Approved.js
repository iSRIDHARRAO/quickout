...
function Approved(){
    ...
    useEffect(()=>{
        setstatus(0)
        axios.get("http://127.0.0.1:3001/deanrequests")
        .then((res)=>{
        ...
        axios.get("http://127.0.0.1:3001/deanaccepted")
        .then((res)=>{
...
function approved(value){
    const url_for_approve = "http://127.0.0.1:3001/deanapproved/"+value
...
function forwarded(value){
    const url_for_approve = "http://127.0.0.1:3001/deandeny/"+value
    axios.get(url_for_approve)
    ...
    return(
        ...
        <div className="applied-values">
           ...
           <div className="number">{value.id}</div>
                ...
                <div className="status"><button type="submit" onClick={()=>forwarded(value.id)}  className="approved">Deny</button></div>
